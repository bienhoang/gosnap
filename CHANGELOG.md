...
- Updated npm package references from `@bienhoang/gosnap` to `gosnap-widget` across README, CHANGELOG, CLI, and landing page

## [1.0.0] - 2026-02-09

### Changed

- **Rebranded from `pro-ui-feedbacks` to `gosnap-widget`** — new npm package name, component exports, web component tag, CLI, and GitHub repo
...
- CLI: `npx pro-ui-feedbacks` → `npx gosnap`
- Props interface: `ProUIFeedbacksProps` → `GoSnapProps`
- IIFE global: `window.ProUIFeedbacks` → `window.GoSnap`

### Breaking

- Package renamed from `pro-ui-feedbacks` to `gosnap-widget`
